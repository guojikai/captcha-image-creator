...
class Pcic
{
  ...
  //create temp image
	...
	private function writeCode() {
      ...
      imagettftext($this->captchaTemp, mt_rand($this->fontSize-5, $this->fontSize+5), mt_rand(-15, 15), $x, $y, $color, self::getFontPath($this->fontFamily), self::csubstr($this->code, $i, 1, false));
		}
	}
  ...
  //create image
    ...
    for($i=0; $i<$this->width; $i++) {
      for ($j=0; $j<$this->height; $j++) {
        ...
        if( (int)($i+5+sin($j/$this->height*2*M_PI)*10) <= imagesx($this->captchaImage) && (int)($i+20+sin($j/$this->height*2*M_PI)*10) >=0 ) {
	 ...
	 * @return  string
	 */
	private static function csubstr($str, $start = 0, $length, $suffix = true, $charset = "utf-8") {
